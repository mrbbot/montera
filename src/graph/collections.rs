...
impl NodeSet {
    ...
    pub fn with_capacity_for<G>(g: &Graph<G>) -> Self {
        let inner = BitSet::with_capacity(g.nodes_len());
        Self { inner }
    }
...
impl FromIterator<NodeId> for NodeSet {
    fn from_iter<T: IntoIterator<Item = NodeId>>(iter: T) -> Self {
        ...
        // extend()'s implementation doesn't use size_hint() itself
...
impl<T> NodeMap<T> {
    ...
    pub fn with_capacity_for<G>(g: &Graph<G>) -> Self {
        ...
        let inner = (0..g.nodes_len()).map(|_| None).collect();
        Self { inner }
    }
...
mod tests {
    ...
    fn map_iter() {
        ...
        // Check all iterators
        assert_eq!(map.iter().collect_vec(), vec![(n1, &10), (n3, &30)]);
        assert_eq!(map.keys().collect_vec(), vec![n1, n3]);
        assert_eq!(map.values().collect_vec(), vec![&10, &30]);
        assert_eq!(map.into_iter().collect_vec(), vec![(n1, 10), (n3, 30)]);
